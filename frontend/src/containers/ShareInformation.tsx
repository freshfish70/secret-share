...
export default function ShareInformation({
  ...
  const info = {
    ...
    retrieval: {
      title: "Bucket",
      text: `${import.meta.env.VITE_BASE_URL}${bucketRoute.viewBucket(
        shareDetails.bucketId
      )}`,
  ...
  return (
    <section className="flex flex-col text-center animate-fadeIn">
      <Snack
        isOpen={copied}
        onCloseHandler={() => setCopied(false)}
        message="Copied"
      />
      <h2>BUCKET CREATED</h2>
      <span className="mb-3 text-xs text-chambray-300 animate-pulse">
        Save these entries{" "}
      </span>
